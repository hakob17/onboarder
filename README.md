...
| **Ticket → fix (Jira / Azure DevOps)** | Connect Jira or ADO (or paste ticket text), and the agent investigates the graph — finds the responsible endpoints, reads each hop's source, highlights the suspect path on the map — then returns an evidence-grounded card: **problem restatement → root cause (`file:line` + node citations) → described fix → a ready-to-paste implementation prompt for a coding agent → risks/unverified**, with a confidence rating. One click can post the analysis back as a ticket comment. No code is written automatically. |
...
| **VS Code / Antigravity extension** | Maps the open folder(s) **with no upload, no Python, and no server to run** — it ships a **bundled, self-contained analysis engine** (a PyInstaller binary launched locally on first use) that reads the folder in place. Status-bar launcher, `Onboarder: Ask about this codebase`, `Onboarder: Analyze a ticket (Jira / ADO)`, CodeLens above mapped code ("2 routes · show in map", "1 writers · 4 readers"), evidence links open files at the line, save-triggered re-analysis. Falls back to the hosted backend if the local engine can't run (`onboarder.engineMode`). Ships as a `.vsix`. |
...
# VS Code extension (standalone — bundles a self-contained engine)
